 ...
 *         C++-laborations by Tommy Olsson and Filip Str√∂mb√§ck.
 */
public class AVLTreeNode<T extends Comparable<T>> {
	...
	public AVLTreeNode<T> insert(T data) {
		...
		int b = checkBalance(this);


		
		if (b > 1 && checkBalance(this.getLeft()) >= 0) { 					
		...
		if (b < -1 && checkBalance(this.getRight()) <= 0) {
	...
	public int checkBalance(AVLTreeNode<T> curr) {
		if (curr == null)
			return 0;

		return nodeHeight(curr.getLeft()) - nodeHeight(curr.getRight());
	...
	public AVLTreeNode<T> remove(T key) {
		...
		int b = checkBalance(this); // ƒr det obalans i nuvarande nod

		if (b > 1 && checkBalance(this.left) >= 0) { // om det ‰r obalans i v‰nster subtr‰d eller hˆger subtr‰d 											// subtr‰det i sig ‰r obalanserat
			return singleRotationWithLeftChild(this);
		}

		if (b > 1 && checkBalance(this.left) < 0) {
			return doubleRotationWithLeftChild(this);
		}

		if (b < -1 && checkBalance(this.right) <= 0) {
			return singleRotationWithRightChild(this);
