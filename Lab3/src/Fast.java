...
public class Fast {
	...
	// Sorting points by natural order (lexicographic order). Makes finding end points of line segments easy.
		...
		ArrayList<Point> slopeList = new ArrayList<Point>(count);
		slopeList.addAll(points);

		for (int i = 0; i < count; i++) { // N gånger
			Point p = points.get(i);
			Collections.sort(slopeList, new FastOrderComparator(p)); // n log n

			for (int l = 0; l < count; l++) { // N gånger
				int x = l + 2;
				if (x < count) {

					if (slopeList.get(l).slopeTo(p) == slopeList.get(x).slopeTo(p)) {
						renderLine(frame, p, slopeList.get(x));
						renderLine(frame, slopeList.get(l), slopeList.get(x));
					}
				}
			}
	 ...
	 * non standard class.
	 */
	private static class FastOrderComparator implements Comparator<Point> {

		private Point p;

		public FastOrderComparator(Point p) {
			this.p = p;
		}

		public int compare(Point a, Point b) {

			if (a.greaterThan(b, p)) {
				return 1;
